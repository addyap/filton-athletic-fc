...
/** Reserves archive hub — links off to each archived season's reserves page. */
    ...
    <div className="min-h-screen bg-white text-slate-900">
      ...
      <section className="border-b border-slate-200 bg-gradient-to-b from-[#1c3f6e] to-[#0a2340] text-white">
        <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 sm:py-16">
          ...
          <h1 className="mt-2 text-2xl font-bold sm:text-3xl lg:text-4xl">Reserves archive</h1>
          <p className="mt-3 max-w-2xl text-sm text-slate-100 sm:text-base">
            Full fixtures, results and final tables from every archived Reserves season.
          </p>
        </div>
      </section>
      ...
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
        ...
        {reserveSeasons.length === 0 ? (
          <div className="mt-6 rounded-lg border border-dashed border-slate-300 bg-white p-8 text-center">
            <p className="font-semibold text-[#0b2d52]">No Reserves seasons archived yet</p>
          </div>
        ) : (
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {reserveSeasons.map((s) => (
              <Link
                key={s.slug}
                to={`/archive/reserves/${s.slug}`}
              ...
              >
                ...
                <p className="mt-2 text-sm text-slate-600">{s.reserveStanding}</p>
...
export default ReservesArchiveHub
